...
def fall_threshold (trace, lo, hi):
    ...
    '''
    print("falling threshold algorithm")
    l = len(trace)
    #send reversed trace
...
def rise_threshold(trace, lo, hi):
    ''' find the threshold crossing points for a trace
        only reliable for smooth data?
        certainly assumes even step size across entire record (e.g. not for log scaling steps!!!!)
        
        Arguments: 
                    -- trace: a 1-D numpy array containing at least one peak
                    -- lo   : the low threshold as a decimal between 0 and 1
                    -- hi   : the high threshold as above

        Returns:    -- nearest value to low threshold
                    -- index of nearest low value
                    -- nearest value to high threshold
                    -- index of nearest high value
                    -- fractional point of low threshold
                    -- fractional point of high threshold
    ...
    if math.fabs(max) < math.fabs(min) :
        peak_negative = True
        #invert trace to simplify maths below
        trace = -trace
        max = trace.max()
        min = trace.min()
    else:
    ...
    print ("peak at index {0} of {1} elements".format(max_idx, l))
...
if __name__ == "__main__":
    ...
    fac = [1, 20, -10, .5]          #testing size, negation
    ...
    for da in data_set:
        ...
        print ("data {0}".format(da))
        print ("{0} - {1}% rise time = {2:.3f} samples".format(int(low * 100), int(high * 100), e - f))
        print ("{0} - {1}% fall time = {2:.3f} samples".format(int(low * 100), int(high * 100), ee - ff))
        print ("Rising a: {0:.3f}, data[b]: {1:.3f}, c: {2:.3f}, data[d]: {3:.3f}".format(a, da[b], c, da[d]))
        print ("Falling aa: {0:.3f}, data[bb]: {1:.3f}, cc: {2:.3f}, data[dd]: {3:.3f}".format(aa, da[bb], cc, da[dd]))
        
        
    a, b, c, d, e, f = rise_threshold (data, .5, .6)       #.6 is not used
    aa, bb, cc, dd, ee, ff = fall_threshold (data, .5, .6)
    print ("{0:f}, {1:f}, {2:f}, {3:f}, {4:f}, {5:f}".format(a, b, c, d, e, f))
    print ("{0:f}, {1:f}, {2:f}, {3:f}, {4:f}, {5:f}".format(aa, bb, cc, dd, ee, ff))
    print ("FWHM = {0:.3f}".format(ee - e))
