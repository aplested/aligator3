...
#module of core utility functions
...
def generate_Q (N_states, drugs, rates, MR_rate, MR_av, MR_needed=True):
    """
    Wrapper for the class that constructs the Q-matrix
    
    Arguments:
    N_states: int
    drugs   : dict must always include keys for each drug-dependent value in the "rates" dict
    rates   : dict of rates
    MR_rate : list of rates to use for MR
    MR_av   : list of rates to avoid, because it would be awkward to have them change
    MR_needed : flag to allow MR calculation to be skipped where unnecessary
    
    Returns:
    The Q-matrix object and the infinite time occupancy vector
    """
    ...
    # if MR has been done on the same Q matrix in a different iteration, MR_needed can be flagged False
...
def convert(period, points_per_decade=5):    
    """convert a time period into log (base 10)-spaced sample points 
