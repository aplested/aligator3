...
def modify_by_hand(rate_dictionary):
    ...
    while 1:
        ...
        if mech_input == '*ls':
            ...
            if b != '*rm':
                if a in rate_dictionary:
                    print("changing rate: %s to %s" %(a,b))
                    rate_dictionary[a] = b

                elif (a in rate_dictionary) == False:
                ...
                elif (a in rate_dictionary) == False and a != None:
...
def chop_HJC_prt (prt, prog='Win_HJCFIT'):
    ...
    if prt.find(open_text) != -1:
        ...
        number_open_states = int(input("Didn't find the number of open states, how many?"))
